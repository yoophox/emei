...
func NewRR(name string, rType uint16, class uint16, ttl uint32, rdLength uint16, rData []byte) *RR {
...
// TrimRRBytes appends bytes from the buffer until it completely parses all the bytes of a resource record.
// It is useful to trim the bytes of a resource record from a buffer.
func TrimRRBytes(buf *bytes.Buffer) []byte {
...
// ToBytes converts the RR to a byte slice.
...
// RRFromBytes creates a RR from a byte slice.
func RRFromBytes(data []byte, messageBufs ...*bytes.Buffer) *RR {
